      ...
      >
        ...
        <Card className="p-8 rounded-[2rem] bg-white/70 dark:bg-slate-900/50 backdrop-blur-2xl border border-white/60 dark:border-white/5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.1)] relative overflow-hidden">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            {errorMsg && (
              <div className="p-3 rounded-xl bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 text-sm font-medium text-center border border-red-100 dark:border-red-500/20">
                {errorMsg}
              </div>
            )}
            ...
            <Input 
              ...
              className="rounded-2xl h-14 bg-white/50 dark:bg-slate-950/50 border-slate-200 dark:border-white/10 focus-visible:ring-indigo-600 font-medium"
              required
            />
            ...
            <div className="relative">
              <Input 
                ...
                className="pr-12 rounded-2xl h-14 bg-white/50 dark:bg-slate-950/50 border-slate-200 dark:border-white/10 focus-visible:ring-indigo-600 font-medium"
                required
              />
              <button 
                type="button" 
            ...
            <Button disabled={loading} type="submit" className="w-full h-14 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold shadow-lg shadow-indigo-600/20 text-md transition-all active:scale-[0.98]">
              {loading ? "Authenticating..." : "Sign In"}
            </Button>
          </form>

          <div className="mt-8 text-center text-sm font-medium text-slate-500">
            Don't have an account? <Link href="/auth/signup" className="text-indigo-600 dark:text-indigo-400 hover:underline font-bold">Create one</Link>
          </div>
        </Card>
      </motion.div>
    </div>
